...
def test_route_uses_calendar_provider(monkeypatch):
    ...
    deps = agent_routes._build_agent_deps(db=MagicMock(), user=user, history_summary="resumo aqui")
...
def test_route_handles_no_calendar_access(monkeypatch):
    ...
    deps = agent_routes._build_agent_deps(db=MagicMock(), user=user, history_summary=None)
...
def test_route_swallows_provider_exception(monkeypatch):
    ...
    monkeypatch.setattr(agent_routes, "build_calendar_access", _boom)
    deps = agent_routes._build_agent_deps(db=MagicMock(), user=user, history_summary=None)
    assert deps.calendar_service is None
