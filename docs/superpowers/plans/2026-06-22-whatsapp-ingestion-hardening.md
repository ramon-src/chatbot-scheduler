...
**Double-run safety:** the sweep can schedule a dispatch for the same orphan from two concurrent webhooks, but `dispatch_agent_run` re-reads `agent_run_at` under its own session and returns early if set — so at most one run persists a turn. The `older_than_minutes` cutoff keeps the in-flight current message out of the sweep. ✓
